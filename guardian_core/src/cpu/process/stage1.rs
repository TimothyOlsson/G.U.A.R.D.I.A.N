...
pub fn update_interconnected_nodes(network: &mut Network) {
    ...
    .for_each(|(neuron_a_index, (neuron_state, node_states, inter_connections))| {
        ...
        for (node_a_local_index, node_a) in node_states.rows().into_iter().enumerate() {
            ...
            let (neuron_state_b,  mut node_b) = if check_is_connected(node_a_global_index, connection_b) {
            ...
            let delta_node_a = &model.forward_from_precalc(&inputs, &precalculated_forward)[DELTA_NODE_A];
            ...
            let delta_node_b = &model.forward_from_precalc(&inputs, &precalculated_backward)[DELTA_NODE_A];
            ...
            for (offset, node_state) in to_write {
